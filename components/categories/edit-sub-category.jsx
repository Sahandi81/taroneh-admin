...
import { useUpdateSubCategoryMutation } from '@/features/api/apiSlice';
import { SUBCATEGORY_EDITED_SUCCESSFULLY } from '@/data/messages';
...
export default function EditSubCategory({ isOpen, closeModal, item }) {
  const [mainId, setMainId] = useState(item.categoryId || null);
  ...
  const defaultSelected = categoriesList.findIndex(item => item.id === mainId);
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (result.success) toast.success(SUBCATEGORY_EDITED_SUCCESSFULLY);
    } catch (error) {
