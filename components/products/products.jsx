...
export default function Products({ header, typeOrWeight }) {
	...
	const { data, error, isLoading } = useGetProductsQuery(page, perPage);
	...
	const memoizedProducts = useMemo(() => {
		if (data) {
			...
			for (let i = 0; i < tmpData.length; i++) {
				productsList[i] = {
					id: tmpData[i]._id,
					name: tmpData[i].title,
					unit: getUnitText(tmpData[i].unit_measurement),
					subCategory: {
						id: tmpData[i]["sub_category"]?._id,
						name: tmpData[i]["sub_category"]?.name,
					},
					inventory: tmpData[i].types[0][0]["package"]["inventory"],
				};
			}
			return productsList;
		}
...
Products.defaultProps = {
	header: ["نام محصول", "زیر شاخه", "واحد", "موجودی انبار", "عملیات"],
	typeOrWeight: [
